...
def _score_matrix(target_batch: List[str], source_list: List[str], scorer: str, workers: int) -> np.ndarray:
    ...
    if scorer == "smart":
        ...
        # Weighted average instead of max: WRatio and ratio are most balanced
        # scorers and are weighted higher. partial_ratio and token_set_ratio are
        # weighted lower to prevent subset matches ("Apple" in "Apple Inc.")
        # from inflating the final score to an undeserved 100.
        weights = np.array([0.20, 0.15, 0.20, 0.15, 0.30], dtype=np.float32)  # ratio, partial, token_sort, token_set, WRatio
...
def fuzzy_match(
    ...
    def load_file(filepath: str) -> pd.DataFrame:
        if filepath.endswith('.csv'):
            if csv_quotechar == '':
                return pd.read_csv(filepath, sep=csv_separator, quoting=3) # quoting=3 means QUOTE_NONE
            return pd.read_csv(filepath, sep=csv_separator, quotechar=csv_quotechar)
